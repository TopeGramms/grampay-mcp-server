export const TOOLS = [
  ...
  },
  {
    ...
    description:
      "Step 1 of 2: validate a USDC→NGN bank payout, verify the recipient's account name, and return a signed token that locks the amount, destination, and idempotency reference for 5 minutes. You debit USDC; the NGN figure is an estimate — IvoryPay sets the real rate at settlement.",
    inputSchema: {
      type: "object",
      properties: {
        amount_usd: {
        ...
        accountNumber: {
          type: "string",
          description:
            "Recipient's 10-digit Nigerian bank account number. Required unless a default account is configured server-side.",
        },
        account_number: {
          type: "string",
