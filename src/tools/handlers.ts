...
export async function handleGetConfig() {
  return {
    ...
    rate_note:
      "USD→NGN is a display estimate only; IvoryPay sets the live settlement rate at payout.",
  };
}
...
export async function handlePrepareCashout(params: {
  ...
  if (!/^\d{10}$/.test(accountNumber)) {
    throw new Error(
      "A valid 10-digit Nigerian bank account number is required " +
...
export async function handleGetStatus(txId: string) {
  ...
  if (CONFIG.MODE === "live") {
    return {
      ...
      status: "UNKNOWN",
      message:
        "Live payout status is delivered via IvoryPay webhooks; durable status lookup lands with the Phase 3 ledger.",
    };
  }
